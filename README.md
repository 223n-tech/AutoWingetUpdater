...
- リソースDLL（`payload\AutoWingetUpdater.dll`）はマニフェストを `mc.exe` でコンパイルして生成したもので、`WEVT_TEMPLATE` などのプロバイダーメタデータを含みます。リポジトリに同梱されており、マニフェストを変更したときは `tools\Build-EventManifestDll.ps1` で再生成します。
- インストール時に `payload\AutoWingetUpdater.man` とリソースDLLを `C:\ProgramData\WingetAutoUpgrade\` に配置し、`wevtutil im` でプロバイダーとチャンネルを登録します。
- 書き込みは `New-WinEvent -ProviderName '223n-tech-AutoWingetUpdater' -Id <id> -Payload @(<message>)` で行います。本文は `Payload` に渡し、イベントの「全般」タブに表示されます。
