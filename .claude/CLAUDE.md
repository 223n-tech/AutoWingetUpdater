...
- **`AutoWingetUpdater.man`（ETWマニフェスト）**
  - `{{RESOURCE_PATH}}` を配置先のリソースDLLパスに置換（XMLエスケープあり）して `UTF-8 BOMなし`で書き出します。
  - リソースDLLは `Add-Type -OutputAssembly` で動的生成します（マニフェストの `resourceFileName`／`messageFileName` の実在チェックを満たすためだけに使う、空の型を1つ含むバイナリ）。
...
- **プロバイダー名は `223n.tech-AutoWingetUpdater`（ハイフン）、チャンネル名は `223n.tech/AutoWingetUpdater`（スラッシュ）**です。チャンネル名のスラッシュがイベントビューアー上の階層区切りになります。両者の表記を取り違えないでください。`Invoke-WingetUpgrade.ps1` の `$ProviderName` はプロバイダー名（ハイフン）と一致させます。
- **プロバイダーGUIDは固定値**です。変更すると別プロバイダー扱いになり旧登録が残るため、安易に変えないでください。
- マニフェストは**メッセージリソースを持たない設計**です。各 `<event>` に `message` 属性を付けず、本文は書き込み時の `-Payload` で渡します。これにより `mc.exe`（メッセージコンパイラ）でのコンパイルが不要になっています。`message` 参照を足すとコンパイル済みリソースDLLが必要になるため、足さないでください。
